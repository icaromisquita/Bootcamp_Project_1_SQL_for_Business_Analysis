...
-- result 409 (based on Icaro’s English Tech list)
...
SELECT pro.product_category_name, pro_trans.product_category_name_english, COUNT(*),
    CASE
        WHEN TIMESTAMPDIFF(day, order_delivered_customer_date, order_estimated_delivery_date) > 0 THEN "delayed"
        ELSE "on time"
    END AS "performance"   
...
GROUP BY product_category_name, performance
