...
SELECT CONCAT(au_fname," ", au_lname, " ") AS au_fullname
FROM authors;
...
FROM authors
WHERE city = ("Oakland" or "Berkeley") and au_fname = "Dean" ;
...
SELECT ROUND(MAX(price),2), pub_id, title
FROM titles
GROUP BY pub_id;

-- 21. Find out the top 3 stores with the most sales
SELECT stor_name, qty
FROM sales, stores
GROUP BY stor_name
ORDER BY qty DESC 
LIMIT 3;
...
SELECT type, COUNT(pub_name), AVG(price)
...
SELECT type, COUNT(pub_name), ROUND(AVG(price),2)
FROM publishers, titles, sales
GROUP BY type
HAVING AVG(price) > 12
